...
if (
  ...
  username = "Anyonymous_" + Math.floor(Math.random() * 1000);
}
...
function makePlayButton(socket_id) {
  let newHTML =
    "<button type='button' class='btn-success'>Play</button>";
  let newNode = $(newHTML);
  newNode.click(() => {
    let payload = {
      requested_user: socket_id
...
function makeStartGameButton() {
  let newHTML =
    "<button type='button' class='btn-danger'>Starting Game</button>";
  let newNode = $(newHTML);
  return newNode;
}
...
socket.on("game_update", (payload) => {
  ...
  for (let row = 0; row < 8; row++) {
    for (let column = 0; column < 8; column++) {
      ...
      if (old_board[row][column] !== board[row][column]) {
        ...
        let altTag = "";
        if ((old_board[row][column] === '?') && (board[row][column] === ' ')) {
          graphic = "EMPTYFinal.gif";
          altTag = "empty space";
        }
        else if ((old_board[row][column] === '?') && (board[row][column] === 'w')) {
          graphic = "EmptytoWhite FINAL.gif";
          altTag = "white token";
        }
        else if ((old_board[row][column] === '?') && (board[row][column] === 'b')) {
          graphic = "EmptytoBlack FINAL.gif";
          altTag = "black token";
        }
        else if ((old_board[row][column] === ' ') && (board[row][column] === 'w')) {
          graphic = "EmptytoWhite FINAL.gif";
          ...
          graphic = "WhitetoEmptyFinal.gif";
          altTag = "empty space";
        }
        else if ((old_board[row][column] === 'b') && (board[row][column] === ' ')) {
          graphic = "BlacktoEmptyFinal.gif";
          altTag = "empty space";
        }
        else if ((old_board[row][column] === 'w') && (board[row][column] === 'b')) {
          graphic = "WhitetoBlackFinal.gif";
          altTag = "black token";
        }
        else if ((old_board[row][column] === 'b') && (board[row][column] === 'w')) {
          graphic = "BlacktoWhiteFinal.gif";
          altTag = "white token";
        }
        else {
          graphic = "ERRORFinal.gif";
          altTag = "error";
        }
  ...
  interval_timer = setInterval(((last_time) => {
    return (() => {
      ...
      $("#elapsed").css("width", total + "%").attr("aria-valnow", total);
